...
Status: **research and local structural checks complete; live skill activation may differ**. Check the repository's current remote and Actions run for publication/CI status; a local test pass does not imply remote CI passed. Installation into any Hermes profile is a separate approval. Real fresh-session activation and cross-model outcome benchmarking remain separate work, not implied by helper tests.
